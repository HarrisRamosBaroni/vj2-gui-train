...
class PreprocessedGUIAgentDataset(Dataset):
    ...
    The dataset expects files in .npz format, each containing 'observations'
    ...
    trajectory (e.g., 16 observations and 7 action blocks).

    Returns a single trajectory of (observations, actions).
            ...
            try:
                with np.load(file_path) as data:
                    if 'observations' in data and 'actions' in data:
                        num_trajectories = len(data['observations'])
                        ...
                        logger.warning(f"Skipping {file_path}: missing 'observations' or 'actions' key.")
    ...
    def __getitem__(self, idx: int):
        ...
        with np.load(file_path) as data:
            observations = data['observations'][local_idx]
            actions = data['actions'][local_idx]

        return torch.from_numpy(observations).float(), torch.from_numpy(actions).float()
...
if __name__ == "__main__":
    ...
    if not Path(processed_dir).exists() or not any(Path(processed_dir).iterdir()):
        ...
        try:
            ...
            # Fetch one batch
            sample_obs, sample_actions = next(iter(test_loader))

            print("\n--- DataLoader Test ---")
            print(f"Batch observations shape: {sample_obs.shape}")
