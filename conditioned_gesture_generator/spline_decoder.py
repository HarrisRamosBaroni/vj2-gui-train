...
def evaluate_spline(control_points: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
    ...
    K = control_points.shape
    
    # Generate a 1D tensor of time points for the control points.
    # The `torchcubicspline` library handles batching internally.
    t_control = torch.linspace(0.0, 1.0, K, device=control_points.device)
    
    # The coefficients will have shape (..., length - 1, channels), so we need to permute our input
    # from (B, K, D) to (B, D, K) to match the library's expectation.
    # control_points_for_coeffs = control_points.transpose(1, 2)
    
    # The `natural_cubic_spline_coeffs` function returns a tuple (t, a, b, c, d)
    # which is then used to initialize the spline.
