...
def train_epoch(model, train_loader, optimizer, loss_fn, quantizer, device, 
    ...
    for batch_idx, data in enumerate(pbar):
        ...
        if should_validate:
            # Determine if we should create visualizations based on the hard limit
            current_step_in_epoch = batch_idx
            should_create_vis = (
                vis_with_val and 
                epoch_vis_count < MAX_VISUALIZATIONS_PER_EPOCH and
                (current_step_in_epoch % vis_interval_steps == 0 or 
                 current_step_in_epoch == total_steps_per_epoch - 1)  # Also vis on last step
            )
            
            if should_create_vis:
                epoch_vis_count += 1
            ...
            val_metrics = validate_epoch(
                ...
                create_visualizations=should_create_vis,
                monitor=monitor
            )
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if val_loader:
            ...
            if run_epoch_val:
                ...
                # Save elite model if validation score qualifies
